...
class Tank_auth
{
	...
	function get_user() {
		...
		return (array) $this->ci->users->get_user_by_id($user_id, "1");
	}
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$profile = $this->ci->users->get_profile();
						$this->ci->session->set_userdata(array(
								'ava'		=> $profile['ava_url'],
						));
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'ava' => '', 'status' => ''));
	...
	function get_ava()
	{
		return $this->ci->session->userdata('ava');
	}

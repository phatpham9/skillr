...
class auth extends CI_Controller
{
	...
	function login()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			//form attributes
			$data['form_open'] = form_open('auth/login', array(
				'class'		=> 'form-signin',
				));
			...
			if ($login_by_username && $login_by_email)
			  ...
			  $login_label = 'Email';
			$data['form_login'] = form_input(array(
				'name'		=> 'login',
				'id'		=> 'login',
			...
			$data['form_remember'] = form_checkbox(array(
				'name'		=> 'remember',
				'id'		=> 'remember',
				'value'		=> 1,
				'checked'	=> set_value('remember'),
				));
			$data['form_captcha'] = form_input(array(
				'name'		=> 'captcha',
				'id'		=> 'captcha',
				'maxlength'	=> 8,
				'class'		=> 'form-control',
			...
			$data['anchor_forgot_password'] = anchor('auth/forgot_password', 'Forgot password');
			if ($this->config->item('allow_registration', 'tank_auth'))
				$data['anchor_register'] = 'Don\'t have an account yet? ' . anchor('auth/register', 'Create an account');
			...
			if ($this->form_validation->run()) {								// validation ok
				if ($this->tank_auth->login(
						...
						$login_by_username,
						$login_by_email)) {										// success
					if ($this->session->userdata('last_page') == '') {
						redirect('');
					} else {													// redirect to last page
				...
				if (form_error('login'))
					$data['error_form_login'] = form_error('login');
				if (form_error('password'))
					$data['error_form_password'] = form_error('password');
				if (form_error('captcha'))
					$data['error_form_captcha'] = form_error('captcha');
			}
			...
			$this->smarty->page('auth/login', $data);
		}
	}
	...
	function forgot_password()
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			//form attributes
			$data['form_open'] = form_open('auth/forgot_password', array(
				'class'		=> 'form-signin',
				));
			$data['form_login'] = form_input(array(
				'name'		=> 'login',
			...
			$data['anchor_login'] = anchor('auth/login', 'Back to log in');
			$data['form_close'] = form_close();
			...
			//validate
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data2 = $this->tank_auth->forgot_password(set_value('login')))) {
					$data2['site_name'] = $this->config->item('website_name', 'tank_auth');

					// Send email with password activation link
					$this->_send_email('forgot_password', $data2['email'], $data2);
			...
			$this->smarty->page('auth/forgot_password', $data);
		}
	}
	...
	function reset_password()
	{
		...
		//form attributes
		$data['form_open'] = form_open($this->uri->uri_string(), array(
			'class'		=> 'form-signin',
			));
		...
		if ($this->form_validation->run()) {								// validation ok
			...
			if (form_error('new_password'))
				$data['error_form_new_password'] = form_error('new_password');
			if (form_error('confirm_new_password'))
				$data['error_form_confirm_new_password'] = form_error('confirm_new_password');
		...
		$this->smarty->page('auth/reset_password', $data);
	}
	...
	function _show_message($message)
	{
		...
		redirect('');	//default redirect('/auth/');
	}
